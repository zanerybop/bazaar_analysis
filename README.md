...
1. Скачайте список рецептов с помощью утилиты (по умолчанию он фильтруется по товарам, представленным на базаре). Клиент последовательно пытается получить данные с официального API `GET /resources/skyblock/recipes`, а при недоступности ресурса переключается на открытый зеркальный дамп из репозитория [HypixelDev/PublicAPI](https://github.com/HypixelDev/PublicAPI). При необходимости можно вручную указать URL основного и резервных источников через флаги `--recipes-api-url` и `--recipes-fallback-url`:
